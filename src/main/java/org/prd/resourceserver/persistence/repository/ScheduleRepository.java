...
public interface ScheduleRepository extends JpaRepository<DoctorSchedule, Long> {
  ...
  @Query("""
    ...
    WHERE ds.specialty.id = :specialtyId
      ...
      AND (
            (:selectedDate IS NULL AND
                CURRENT_DATE BETWEEN ds.startDate AND ds.endDate AND
                FUNCTION('MONTH', ds.startDate) = FUNCTION('MONTH', CURRENT_DATE) AND
                FUNCTION('YEAR', ds.startDate) = FUNCTION('YEAR', CURRENT_DATE)
            )
            OR
            (:selectedDate IS NOT NULL AND
                :selectedDate BETWEEN ds.startDate AND ds.endDate AND
                FUNCTION('MONTH', ds.startDate) = FUNCTION('MONTH', :selectedDate) AND
                FUNCTION('YEAR', ds.startDate) = FUNCTION('YEAR', :selectedDate)
            )
          )
""")
